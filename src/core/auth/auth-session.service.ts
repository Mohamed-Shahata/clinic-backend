...
export class AuthSessionService {
  ...
  async refresh(refreshToken: string) {
    const refreshHash = this.hashToken(refreshToken);
    const raw = await this.cacheManager.get<string>(this.refreshKey(refreshHash));
  ...
  async revokeAccessToken(jti: string | undefined, expiresAtSeconds?: number) {
    ...
    const ttlMs = Math.max(
      1,
      ((expiresAtSeconds ?? Math.floor(Date.now() / 1000) + ACCESS_TOKEN_TTL_SECONDS) -
  ...
  async isAccessTokenRevoked(jti?: string) {
    if (!jti) return true;
    return (await this.cacheManager.get<string>(this.revokedAccessKey(jti))) === "1";
  }

  async revokeClinicSessions(clinicId: string) {
    await this.cacheManager.set(this.clinicRevokedKey(clinicId), "1", REFRESH_TOKEN_TTL_MS);
  }
  ...
  async isClinicRevoked(clinicId?: string) {
    ...
    return (await this.cacheManager.get<string>(this.clinicRevokedKey(clinicId))) === "1";
  }
