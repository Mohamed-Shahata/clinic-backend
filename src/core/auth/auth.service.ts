...
export class AuthService {
  ...
  // HIGH-03: Use typed Prisma call instead of raw SQL
  ...
  async login(dto: LoginDto) {
    ...
    // HIGH-02: Use findUnique on indexed unique fields where possible
    const user =
    ...
    if (!user.isSuperAdmin) {
      ...
      if (slugFilter) {
        ...
        if (!membership || !(await this.isClinicActive(membership.clinicId))) {
        ...
        for (const m of memberships) {
          if (await this.isClinicActive(m.clinicId)) activeMemberships.push(m);
        }
  ...
  async logout(refreshToken: string | undefined, accessJti?: string, accessExp?: number) {
    await Promise.all([
  ...
  async requestForgotPassword(dto: ForgotPasswordRequestDto) {
    ...
    // CRIT-01: use cache (Redis-backed in production) instead of in-memory Map
    ...
    await this.cacheManager.set(`forgot:${email}`, JSON.stringify({ code }), OTP_TTL_MS);
  ...
  async resetForgotPassword(dto: ForgotPasswordResetDto) {
    ...
    if (code !== dto.code) throw new BadRequestException("Invalid or expired reset code");
  ...
  async confirmEmailChange(userId: string, dto: ConfirmEmailChangeDto) {
    ...
    const { code, newEmail } = JSON.parse(raw) as { code: string; newEmail: string };
    if (code !== dto.code) throw new BadRequestException("Invalid or expired verification code");
